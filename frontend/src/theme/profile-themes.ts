...
export interface ProfileTheme {
  ...
  spacing: 'compact' | 'medium' | 'generous' | 'dense' | 'premium'
  iconSize: string
  avatarSize: number
  cardStyle: {
    background: string
...
export const getProfileColor = (profile: string): string => {
  return getProfileTheme(profile).primaryColor
}

export const getProfileFontSize = (profile: string, size: keyof ProfileTheme['textSize'] = 'medium'): string => {
  return getProfileTheme(profile).textSize[size]
}

export const getProfileSpacing = (profile: string): ProfileTheme['spacing'] => {
  return getProfileTheme(profile).spacing
}

export const getProfileIconSize = (profile: string): string => {
  return getProfileTheme(profile).iconSize
}

export const getProfileAvatarSize = (profile: string): number => {
  return getProfileTheme(profile).avatarSize
}

export const getProfileCardStyle = (profile: string) => {
